...
def fill_travel_destinations_from_xls(file_path):
    ...
    for row_index in range(1, sheet.nrows):
        ...
        province=sheet.cell_value(row_index, 2).rstrip("省")
        city=sheet.cell_value(row_index, 3).rstrip("市")
        address = sheet.cell_value(row_index, 6)  # 第18项
        tele=sheet.cell_value(row_index, 7)
        description=address+"\n"+tele
        popularity = random.randint(0, 10000)  # 生成随机整数
        rating = round(random.uniform(3, 5), 1)  # 生成随机一位小数
        image_url = sheet.cell_value(row_index, 11)  # 第九项
        ...
        nature_category = Category.objects.get(name="学院高校")
...
if __name__ == "__main__":
    # 传入 xls 文件的路径
    xls_file_path = "tool/university.xls"
